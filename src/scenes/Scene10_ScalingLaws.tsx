...
const ScalingGraph: React.FC<{ sceneDurationFrames: number }> = ({ sceneDurationFrames }) => {
  ...
  const axisDurationFrames = 18;
const axisDraw = spring({ frame, fps, config: SPRINGS.flow, durationInFrames: axisDurationFrames });

const draw = spring({
  frame: Math.max(frame - axisDurationFrames, 0),
  fps,
  config: SPRINGS.flow,
  ...
  strokeDasharray={GRAPH_W}
  strokeDashoffset={interpolate(axisDraw, [0, 1], [GRAPH_W, 0])}
/>
<line
  x1={0} y1={GRAPH_H} x2={0} y2={0}
  stroke="rgba(255,255,255,0.5)" strokeWidth={3}
  strokeDasharray={GRAPH_H}
  strokeDashoffset={interpolate(axisDraw, [0, 1], [GRAPH_H, 0])}
/>
...
/>
  ...
  fill={GRAPH_WHITE}
  style={{ filter: `drop-shadow(0 0 16px ${GRAPH_YELLOW})`, opacity: draw }}
/>
